...
export default function NewsMedicalTourism() {
    ...
    const UI = {
        uz: {
            title: 'Tibbiy turizm yangiliklari',
            desc: 'Tibbiy turizm bo‘yicha so‘nggi yangiliklar, hamkorlik dasturlari va foydali ma’lumotlar bilan tanishing.',
            empty: 'Hozircha tibbiy turizm bo‘yicha yangiliklar topilmadi.',
        },
        ru: {
            title: 'Новости медицинского туризма',
            desc: 'Следите за последними новостями, партнёрскими программами и полезной информацией в сфере медтуризма.',
            empty: 'Пока нет новостей по медицинскому туризму.',
        },
        en: {
            title: 'Medical Tourism News',
            desc: 'Discover the latest updates, partnerships, and helpful information in medical tourism.',
            empty: 'No medical tourism news available yet.',
        },
    };
    ...
    const getData = async () => {
        try {
            const res = await ApiResult.getMedTourism();
            setRaw(res?.results);
        } catch {
            setRaw([]);
        }
