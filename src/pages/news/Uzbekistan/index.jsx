...
export default function NewsUz() {
    ...
    const UI = {
        uz: {
            ...
            desc: 'Mamlakatimizdagi eng so‘nggi yangiliklar, rasmiy bayonotlar va dolzarb voqealar bilan tanishing.',
            empty: 'Hozircha yangiliklar topilmadi.',
        },
        ru: {
            title: 'Новости Узбекистана',
            desc: 'Следите за последними новостями, официальными заявлениями и актуальными событиями страны.',
            empty: 'Пока нет новостей.',
        },
        en: {
            title: 'Uzbekistan News',
            desc: 'Stay updated with the latest national news, official statements, and trending events in Uzbekistan.',
    ...
    const getData = async () => {
        try {
            const res = await ApiResult.getUzbekistan();
            setRaw(res?.results);
        } catch {
            setRaw([]);
        }
