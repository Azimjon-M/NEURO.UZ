...
export default function GalCongress3() {
    const { language } = Languages(); // 'uz' | 'ru' | 'en'
    const [data, setData] = useState(null);

    const UI = {
        uz: {
            pageTitle: '3 - Kongresdan lavhalar',
            empty: "Ma'lumot yuklanmadi",
            noMedia: 'Media yo‘q',
        },
        ru: {
            pageTitle: '3 - Выдержки из Конгресса',
            empty: 'Данные не загружены',
            noMedia: 'Медиа нет',
        },
        en: {
            pageTitle: '3 - Clips from the Congress',
    ...
    const getData = async () => {
        try {
            const res = await ApiResult.getGalery3();
            setData(res?.results || []);
        } catch {
            setData([]);
        }
