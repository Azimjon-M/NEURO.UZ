// src/pages/education/EduDissertation.jsx
import React, { useCallback, useEffect, useMemo, useState } from 'react';
import { FiChevronDown, FiDownload } from 'react-icons/fi';
import ApiResult from '@/services/scienceEducation'; // endpointni keyin o'zingiz almashtirasiz
...
const UI_T = {
    uz: {
        ...
        fallbackTitle: 'Dissertatsiya kengashi',
        fallbackDesc:
            'Dissertatsiya kengashi tarkibi, muddatlar, taqdim etish tartibi va tegishli hujjatlar bilan tanishing.',
    },
    ru: {
        openFile: 'Открыть файл',
        openAttachment: 'Открыть вложение',
        empty: 'Данные не загружены',
        fallbackTitle: 'Диссертационный совет',
        fallbackDesc:
            'Ознакомьтесь с составом совета, сроками, порядком подачи и связанными документами.',
    },
    en: {
        openFile: 'Open file',
        openAttachment: 'Open attachment',
        empty: 'No data loaded',
        fallbackTitle: 'Dissertation Council',
        fallbackDesc:
            'Learn about the council members, timelines, submission procedures, and related documents.',
    },
};
...
/** Matndan til markerlarini olib tashlash:
 *  - "[RU] ...." / "[EN] ...." / "[UZ] ...."
 *  - "RU: ...." / "EN - ...." / "UZ – ...."
 */
const stripLangTag = (val) => {
    ...
    let s = val.trim();
    s = s.replace(/^\s*\[(?:uz|ru|en)\]\s*/i, ''); // [UZ]/[RU]/[EN]
    s = s.replace(/^\s*(?:UZ|RU|EN)\s*[:\-–]\s*/i, ''); // UZ:/RU:/EN: yoki UZ- …
    return s;
};

/* ================= Ichki Accordion (EduEvents kloni) ================= */
...
const EduDissertation = () => {
    ...
    // Tilga mos qiymat tanlash (obj || string) + markerlarni tozalash
    ...
    const getData = useCallback(async () => {
        try {
            const response = await ApiResult.getDissertatsiyaKengashi();
...
export default EduDissertation;
